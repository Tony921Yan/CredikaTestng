...
class ItemDetailPictureTest extends BaseTest {
    ...
    TestContext[] data(){
        ...
        List  picList = MysqlAPI.platformGoodsSql.rows("select code,detail from goods_spu where status = 4 and gmt_modified > date_sub(curdate(),interval 1 day) order by gmt_modified desc limit 50")
        picList.forEach{it ->
